...
const ecosystemDocFile = path.join(
    __dirname,
    '..',
    '..',
    basePathEcosystemDocFile,
);
const failureTypes = {
    ...
    if (isError) {
        failures.forEach((failure) => {
            if (failure.type === failureTypes.improperFormat) {
                core.error(
                    'The module name should be enclosed with backticks',
                    {
                ...
                core.error(
                    `${failure.moduleName} not listed in alphabetical order`,
                    {
                        title: 'Out of Order',
                        file: basePathEcosystemDocFile,
                        startLine: failure.lineNumber,
