...
function createServer(options, httpHandler) {
    ...
    function listen(listenOptions, ...args) {
        ...
        if (arguments.length === 0) {
            ...
            arguments.length > 0 &&
            firstArgType !== '[object Object]' &&
            firstArgType !== '[object Function]'
        ) {
            FSTDEP011();
            ...
            Object.assign(
                listenOptions,
                normalizeListenArgs([...hostPath, ...args]),
            );
        } else {
            listenOptions.cb = cb;
        }
        if (listenOptions.signal) {
            if (
                typeof listenOptions.signal.on !== 'function' &&
                typeof listenOptions.signal.addEventListener !== 'function'
            ) {
                throw new FST_ERR_LISTEN_OPTIONS_INVALID(
                    'Invalid options.signal',
                );
            }
            ...
            if (listenOptions.signal.aborted) {
                ...
                listenOptions.signal.addEventListener('abort', onAborted, {
                    once: true,
                });
            }
        }
        ...
        if (
            Object.prototype.hasOwnProperty.call(listenOptions, 'host') ===
            false
        ) {
...
function multipleBindings(
    ...
    dns.lookup(listenOptions.host, {all: true}, (dnsErr, addresses) => {
        ...
        const isMainServerListening =
            mainServer.listening && serverOpts.serverFactory;
        ...
        if (!isMainServerListening) {
            ...
            for (const adr of addresses) {
                if (adr.address !== primaryAddress.address) {
                    ...
                    const secondaryServer = getServerInstance(
                        serverOpts,
                        httpHandler,
                    );
                    const closeSecondary = () => {
                        ...
                        if (serverOpts.forceCloseConnections === 'idle') {
                            ...
                            typeof secondaryServer.closeAllConnections ===
                                'function' &&
                            serverOpts.forceCloseConnections
                        ) {
                    ...
                    secondaryServer.on(
                        'upgrade',
                        mainServer.emit.bind(mainServer, 'upgrade'),
                    );
    ...
    const wrap = (err) => {
        ...
        if (!err) {
            const address = logServerAddress.call(
                this,
                server,
                listenOptions.listenTextResolver ||
...
function logServerAddress(server, listenTextResolver) {
    ...
    address =
        (isUnixSocket
            ? ''
            : 'http' + (this[kOptions].https ? 's' : '') + '://') + address;
