...
function normalizeSchema(routeSchemas, serverOptions) {
    ...
    for (const key of SCHEMAS_SOURCE) {
        ...
        if (schema && !isCustomSchemaPrototype(schema)) {
            routeSchemas[key] = getSchemaAnyway(
                schema,
        ...
        for (const code of httpCodes) {
            ...
            if (contentProperty) {
                ...
                for (let i = 0; i < keys.length; i++) {
                    ...
                    routeSchemas.response[code].content[mediaName].schema =
                        getSchemaAnyway(
...
function getSchemaSerializer(context, statusCode, contentType) {
    ...
    if (responseSchemaDef[statusCode]) {
        if (
            responseSchemaDef[statusCode].constructor === Object &&
            contentType
        ) {
    ...
    if (responseSchemaDef[fallbackStatusCode]) {
        if (
            responseSchemaDef[fallbackStatusCode].constructor === Object &&
            contentType
        ) {
            const mediaName = contentType.split(';', 1)[0];
