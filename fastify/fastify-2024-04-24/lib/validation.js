...
const {FST_ERR_SCH_RESPONSE_SCHEMA_NOT_NESTED_2XX} = require('./errors');
...
function compileSchemasForSerialization(context, compile) {
    ...
    context[responseSchema] = Object.keys(context.schema.response).reduce(
...
function compileSchemasForValidation(context, compile, isCustom) {
    ...
    if (
        headers &&
        (isCustom || Object.getPrototypeOf(headers) !== Object.prototype)
    ) {
        ...
        if (headersSchemaLowerCase.required instanceof Array) {
            headersSchemaLowerCase.required =
                headersSchemaLowerCase.required.map((h) => h.toLowerCase());
        }
        if (headers.properties) {
            headersSchemaLowerCase.properties = {};
            Object.keys(headers.properties).forEach((k) => {
                headersSchemaLowerCase.properties[k.toLowerCase()] =
                    headers.properties[k];
            });
        }
        context[headersSchema] = compile({
...
function validateParam(validatorFunction, request, paramName) {
    ...
    const ret =
        validatorFunction &&
        validatorFunction(isUndefined ? null : request[paramName]);
    ...
    if (ret?.then) {
        return ret
            ...
            .catch((err) => {
                return err;
            }); // return as simple error (not throw)
    }
...
function validate(context, request, execution) {
    ...
    if (runExecution || !execution.skipQuery) {
        const query = validateParam(
            context[querystringSchema],
            request,
            'query',
        );
        if (query) {
