...
Reply.prototype.getSerializationFunction = function (
    schemaOrStatus,
    contentType,
) {
    let serialize;
    ...
    if (
        ...
        serialize =
            this[kRouteContext][kReplyCacheSerializeFns]?.get(schemaOrStatus);
    }
...
Reply.prototype.serializeInput = function (
    input,
    schema,
    httpStatus,
    contentType,
    ...
    contentType =
        httpStatus && possibleContentType !== httpStatus
            ? possibleContentType
            : contentType;
    ...
    if (httpStatus != null) {
        ...
        if (serialize == null) {
            if (contentType)
                throw new FST_ERR_MISSING_CONTENTTYPE_SERIALIZATION_FN(
                    httpStatus,
                    contentType,
                );
        ...
        if (this[kRouteContext][kReplyCacheSerializeFns]?.has(schema)) {
            serialize =
                this[kRouteContext][kReplyCacheSerializeFns].get(schema);
        } else {
            serialize = this.compileSerializationSchema(
                schema,
                httpStatus,
                contentType,
            );
...
Reply.prototype.serialize = function (payload) {
    if (this[kReplySerializer] !== null) {
        ...
        if (
            this[kRouteContext] &&
            this[kRouteContext][kReplySerializerDefault]
        ) {
            return this[kRouteContext][kReplySerializerDefault](
                payload,
                this.raw.statusCode,
            );
        } else {
...
function preSerializationHookEnd(err, request, reply, payload) {
    ...
    try {
        if (reply[kReplySerializer] !== null) {
            ...
            reply[kRouteContext] &&
            reply[kRouteContext][kReplySerializerDefault]
        ) {
            payload = reply[kRouteContext][kReplySerializerDefault](
                payload,
...
function onErrorHook(reply, error, cb) {
    if (
        reply[kRouteContext].onError !== null &&
        !reply[kReplyNextErrorHandler]
    ) {
