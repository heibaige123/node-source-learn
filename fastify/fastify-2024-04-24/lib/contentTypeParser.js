...
const {
    ...
    defaultContentType,
} = require('fast-content-type-parse');
...
function ContentTypeParser(
    ...
    this.customParsers.set(
        'application/json',
        new Parser(true, false, bodyLimit, this[kDefaultJsonParse]),
    );
    this.customParsers.set(
        'text/plain',
        new Parser(true, false, bodyLimit, defaultPlainTextParser),
    );
    this.parserList = [
...
ContentTypeParser.prototype.hasParser = function (contentType) {
    return this.customParsers.has(
        typeof contentType === 'string' ? contentType : contentType.toString(),
    );
};
...
ContentTypeParser.prototype.existingParser = function (contentType) {
    if (
        ...
        this.customParsers.has(contentType)
    ) {
        return (
...
ContentTypeParser.prototype.remove = function (contentType) {
    ...
    const idx = parsers.findIndex(
        (ct) => ct.toString() === contentType.toString(),
    );
...
ContentTypeParser.prototype.run = function (
    ...
    if (parser === undefined) {
        if (request.is404) {
            ...
            reply.send(
                new FST_ERR_CTP_INVALID_MEDIA_TYPE(contentType || undefined),
            );
        }
...
function removeContentTypeParser(contentType) {
    if (this[kState].started) {
        throw new FST_ERR_CTP_INSTANCE_ALREADY_STARTED(
