...
const {FSTDEP007, FSTDEP008, FSTDEP014} = require('./warnings');
...
function buildRouting(options) {
    ...
    return {
        ...
        setup(options, fastifyArgs) {
            ...
            return503OnClosing = Object.prototype.hasOwnProperty.call(
                options,
                'return503OnClosing',
            )
    ...
    function route({options, isFastify}) {
        ...
        const shouldExposeHead = hasRouteExposeHeadRouteFlag
            ? exposeHeadRoute
        ...
        function addNewRoute({path, prefixing = false, isFastify = false}) {
            ...
            for (const hook of lifecycleHooks) {
                if (opts && hook in opts) {
                    if (Array.isArray(opts[hook])) {
                        ...
                        opts[hook] !== undefined &&
                        typeof opts[hook] !== 'function'
                    ) {
                        throw new FST_ERR_HOOK_INVALID_HANDLER(
                            hook,
                            Object.prototype.toString.call(opts[hook]),
                        );
                    }
                }
            }
            ...
            try {
                ...
                if (!context[kRouteByFastify]) {
                    const isDuplicatedRoute = error.message.includes(
                        `Method '${opts.method}' already declared for route '${opts.url}'`,
                    );
                    if (isDuplicatedRoute) {
                        throw new FST_ERR_DUPLICATED_ROUTE(
                            opts.method,
                            opts.url,
                        );
                    }
            ...
            this.after((notHandledErr, done) => {
                ...
                context[kReplySerializerDefault] =
                ...
                avvio.once('preReady', () => {
                    ...
                    while (
                        !context.Request[kHasBeenDecorated] &&
                        context.Request.parent
                    ) {
                        context.Request = context.Request.parent;
                    }
                    while (
                        !context.Reply[kHasBeenDecorated] &&
                        context.Reply.parent
                    ) {
                    ...
                    if (opts.schema) {
                        ...
                        try {
                            const isCustom =
                                typeof opts?.validatorCompiler === 'function' ||
    ...
    function routeHandler(req, res, params, context, query) {
        ...
        const connHeader = String.prototype.toLowerCase.call(
...
function validateSchemaBodyOption(method, path, schema) {
    if ((method === 'GET' || method === 'HEAD') && schema && schema.body) {
        throw new FST_ERR_ROUTE_BODY_VALIDATION_SCHEMA_NOT_SUPPORTED(
            method,
            path,
        );
    }
...
function runPreParsing(err, request, reply) {
    ...
    if (request[kRouteContext].preParsing !== null) {
        preParsingHookRunner(
            request[kRouteContext].preParsing,
            request,
            reply,
            handleRequest,
        );
