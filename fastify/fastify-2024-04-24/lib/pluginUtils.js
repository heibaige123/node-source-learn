...
const {kTestInternals} = require('./symbols.js');
...
function checkDependencies(fn) {
    ...
    assert(
        ...
        'The dependencies should be an array of strings',
    );
...
function _checkDecorators(that, instance, decorators, name) {
    assert(
        ...
        'The decorators should be an array of strings',
    );

    decorators.forEach((decorator) => {
        const withPluginName =
            typeof name === 'string' ? ` required by '${name}'` : '';
        if (!checks[instance].call(that, decorator)) {
            throw new FST_ERR_PLUGIN_NOT_PRESENT_IN_INSTANCE(
                decorator,
                withPluginName,
                instance,
            );
...
function checkVersion(fn) {
    ...
    if (
        fastifyRc === true &&
        semver.gt(this.version, semver.coerce(requiredVersion)) === true
    ) {
    ...
    if (
        requiredVersion &&
        semver.satisfies(this.version, requiredVersion, {
            includePrerelease: fastifyRc,
        }) === false
    ) {
        ...
        // throw an error.
        throw new FST_ERR_PLUGIN_VERSION_MISMATCH(
            meta.name,
            requiredVersion,
            this.version,
        );
