...
function fastify(options) {
    ...
    if (
        ...
        throw new FST_ERR_SCHEMA_CONTROLLER_BUCKET_OPT_NOT_FN(
            typeof options.schemaController.bucket,
        );
    }
    ...
    if (
        ...
        throw new FST_ERR_AJV_CUSTOM_OPTIONS_OPT_NOT_OBJ(
            typeof ajvOptions.customOptions,
        );
    }
    if (!ajvOptions.plugins || !Array.isArray(ajvOptions.plugins)) {
        throw new FST_ERR_AJV_CUSTOM_OPTIONS_OPT_NOT_ARR(
            typeof ajvOptions.plugins,
        );
    }
    ...
    options.requestTimeout =
        options.requestTimeout || defaultInitOptions.requestTimeout;
    ...
    const router = buildRouting({
        config: {
            ...
            ignoreTrailingSlash:
                ...
                defaultInitOptions.ignoreTrailingSlash,
            ignoreDuplicateSlashes:
                options.ignoreDuplicateSlashes ||
                defaultInitOptions.ignoreDuplicateSlashes,
            maxParamLength:
                options.maxParamLength || defaultInitOptions.maxParamLength,
            caseSensitive: options.caseSensitive,
            allowUnsafeRegex:
                options.allowUnsafeRegex || defaultInitOptions.allowUnsafeRegex,
            buildPrettyMeta: defaultBuildPrettyMeta,
            querystringParser: options.querystringParser,
            useSemicolonDelimiter:
                options.useSemicolonDelimiter ??
                defaultInitOptions.useSemicolonDelimiter,
        },
    });
    ...
    const serverHasCloseAllConnections =
        typeof server.closeAllConnections === 'function';
    const serverHasCloseIdleConnections =
    ...
    const keepAliveConnections =
        !serverHasCloseAllConnections && forceCloseConnections === true
            ? new Set()
            : noopSet();
    ...
    const fastify = {
        ...
        getDefaultJsonParser:
            ContentTypeParser.defaultParsers.getDefaultJsonParser,
        defaultTextParser: ContentTypeParser.defaultParsers.defaultTextParser,
        removeContentTypeParser:
            ContentTypeParser.helpers.removeContentTypeParser,
        ...
        hasPlugin: function (name) {
            return (
                this[pluginUtils.kRegisteredPlugins].includes(name) ||
                this[kPluginNameChain].includes(name)
            );
        },
        // http server
        listen,
    ...
    if (options.schemaErrorFormatter) {
        ...
        fastify[kSchemaErrorFormatter] =
            options.schemaErrorFormatter.bind(fastify);
    }
    ...
    const avvio = Avvio(fastify, {
        autostart: false,
        timeout:
            isNaN(avvioPluginTimeout) === false
                ? avvioPluginTimeout
    ...
    function ready(cb) {
        ...
        function runHooks() {
            ...
            fastify[kAvvioBoot]((err, done) => {
                if (
                    err ||
                    fastify[kState].started ||
                    fastify[kState].ready ||
                    fastify[kState].booting
                ) {
                    manageErr(err);
                } else {
        ...
        return function preRouting(req, res) {
            ...
            if (rewriteUrl) {
                ...
                if (typeof url === 'string') {
                    ...
                    const err = new FST_ERR_ROUTE_REWRITE_NOT_STR(
                        req.url,
                        typeof url,
                    );
                    req.destroy(err);
            ...
            router.routing(
                req,
                res,
                buildAsyncConstraintCallback(isAsync, req, res),
            );
        };
    }
...
function validateSchemaErrorFormatter(schemaErrorFormatter) {
    if (typeof schemaErrorFormatter !== 'function') {
        throw new FST_ERR_SCHEMA_ERROR_FORMATTER_NOT_FN(
