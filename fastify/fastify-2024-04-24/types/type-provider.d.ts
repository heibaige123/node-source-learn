...
export type CallTypeProvider<F extends FastifyTypeProvider, I> = (F & {
    input: I;
})['output'];
...
type ResolveRequestQuerystring<
    ...
    RouteGeneric extends RouteGenericInterface,
> = UndefinedToUnknown<
    KeysOf<RouteGeneric['Querystring']> extends never
...
type ResolveRequestBody<
    ...
    RouteGeneric extends RouteGenericInterface,
> = UndefinedToUnknown<
    KeysOf<RouteGeneric['Body']> extends never
...
> = {
    [K1 in keyof SchemaCompiler['response']]: SchemaCompiler['response'][K1] extends {
        ...
        ? {
              [K2 in keyof SchemaCompiler['response'][K1]['content']]: CallTypeProvider<
                  TypeProvider,
                  SchemaCompiler['response'][K1]['content'][K2]['schema']
              >;
          } extends infer Result
            ? Result[keyof Result]
            : unknown
        : CallTypeProvider<TypeProvider, SchemaCompiler['response'][K1]>;
} extends infer Result
    ? Result[keyof Result]
    : unknown;
