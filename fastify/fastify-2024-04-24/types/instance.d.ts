...
type NotInInterface<Key, _Interface> = Key extends keyof _Interface
    ? never
    : Key;
type FindMyWayVersion<RawServer extends RawServerBase> =
    RawServer extends http.Server ? HTTPVersion.V1 : HTTPVersion.V2;
type FindMyWayFindResult<RawServer extends RawServerBase> = FindResult<
...
> {
    ...
    > &
        PromiseLike<undefined>;
    ready(
        readyListener: (err: Error | null) => void,
    ): FastifyInstance<RawServer, RawRequest, RawReply, Logger, TypeProvider>;
    ...
    addHook<
        Fn extends
            | onReadyHookHandler
            | onReadyAsyncHookHandler = onReadyHookHandler,
    >(
        name: 'onReady',
        hook: Fn extends unknown
            ? Fn extends AsyncFunction
    ...
    addHook<
        Fn extends
            | onCloseHookHandler
            | onCloseAsyncHookHandler = onCloseHookHandler,
    >(
        name: 'onClose',
        hook: Fn extends unknown
            ? Fn extends AsyncFunction
    ...
    addHook<
        Fn extends
            | preCloseHookHandler
            | preCloseAsyncHookHandler = preCloseHookHandler,
    >(
        name: 'preClose',
        hook: Fn extends unknown
            ? Fn extends AsyncFunction
    ...
    setChildLoggerFactory(
        factory: FastifyChildLoggerFactory<
            RawServer,
            RawRequest,
            RawReply,
            Logger,
     ...
     */
    setValidatorCompiler<T = FastifySchema>(
     ...
     */
    setSerializerCompiler<T = FastifySchema>(
