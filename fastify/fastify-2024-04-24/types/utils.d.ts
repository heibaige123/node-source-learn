...
export type ReplyKeysToCodes<Key> = [Key] extends [never]
    ? number
    : Key extends HttpCodes
      ? Key
      : Key extends `${infer X extends CodeClasses}xx`
        ? StringAsNumber<`${X}${Digit}${Digit}`>
        : number;

export type CodeToReplyKey<Code extends number> =
    `${Code}` extends `${infer FirstDigit extends CodeClasses}${number}`
        ? `${FirstDigit}xx`
        : never;

export type RecordKeysToLowercase<Input> =
