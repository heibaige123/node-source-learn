...
test('abort signal', {skip: semver.lt(process.version, '16.0.0')}, (t) => {
    ...
    t.test('listen should throw if received invalid signal', (t) => {
        ...
        try {
            ...
            t.equal(
                e.message,
                "Invalid listen options: 'Invalid options.signal'",
            );
        }
    });
...
t.test(
    '#5180 - preClose should be called before closing secondary server',
    (t) => {
        t.plan(2);
        ...
        fastify.listen({port: 0}, (err) => {
            ...
            undici
                .request(
                    `http://${secondaryAddress.address}:${secondaryAddress.port}/`,
                )
                .then(
                    () => {
