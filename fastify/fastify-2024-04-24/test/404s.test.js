...
test('setting a custom 404 handler multiple times is an error', (t) => {
    ...
    t.test('at the root level', (t) => {
        ...
        try {
            fastify.setNotFoundHandler(() => {});
            t.fail(
                'setting multiple 404 handlers at the same prefix encapsulation level should throw',
            );
        } catch (err) {
            t.type(err, Error);
            t.equal(
...
test('setNotFoundHandler should not suppress duplicated routes checking', (t) => {
    ...
    try {
        ...
        t.fail(
            'setNotFoundHandler should not interfere duplicated route error',
        );
    } catch (error) {
        t.ok(error);
    }
    ...
    t.test('log debug', (t) => {
        ...
        fastify.inject(
            ...
            (err, response) => {
                ...
                t.equal(
                    JSON.parse(logStream.logs[1]).msg,
...
test('preHandler option for setNotFoundHandler', (t) => {
    ...
    t.test(
        'preHandler hook in setNotFoundHandler should be called when callNotFound',
        {timeout: 40000},
        (t) => {
            t.plan(3);
            const fastify = Fastify();
    ...
    t.test(
        'preHandler hook in setNotFoundHandler should accept an array of functions and be called when callNotFound',
        (t) => {
            t.plan(2);
            const fastify = Fastify();
...
test('setNotFoundHandler should be chaining fastify instance', (t) => {
    t.test('Register route after setNotFoundHandler', (t) => {
        ...
        const fastify = Fastify();
        fastify
            .setNotFoundHandler(function (_req, reply) {
                reply.code(404).send('this was not found');
            })
    ...
    t.test('Dynamic route', (t) => {
        ...
        const fastify = Fastify({
            frameworkErrors: (error, req, reply) => {
                t.equal(
                    error.message,
                    "'/hello/%world' is not a valid url component",
                );
