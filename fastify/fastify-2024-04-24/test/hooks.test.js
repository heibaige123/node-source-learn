...
test('hooks', (t) => {
    ...
    try {
        ...
        t.equal(
            e.message,
            'preHandler hook should be a function, instead got null',
        );
        t.pass();
    }
    ...
    try {
        ...
        t.equal(
            e.message,
            'preParsing hook should be a function, instead got undefined',
        );
        t.pass();
    }
    ...
    try {
        fastify.addHook(
            'preSerialization',
            function (request, reply, payload, done) {
...
test('onRoute hook should be called once when prefixTrailingSlash', (t) => {
    ...
    fastify.register(
        fp(function myPlugin(instance, opts, next) {
    ...
    fastify.register(
        function routes(instance, opts, next) {
            instance.route({
                method: 'GET',
                url: '/',
...
test('onSend hook is called after payload is serialized and headers are set', (t) => {
    ...
    fastify.register((instance, opts, done) => {
        ...
        instance.addHook('onSend', function (request, reply, payload, done) {
            ...
            t.equal(
                reply[symbols.kReplyHeaders]['content-type'],
        ...
        './lib/schemas.js': {
            getSchemaSerializer: (param1, param2, param3) => {
                t.equal(
                    param3,
                    'application/json; charset=utf-8',
                    'param3 should be "application/json; charset=utf-8"',
                );
            },
        },
    });
    ...
    fastify.inject(
        {
            ...
            payload: {
                message: Buffer.from(
                    JSON.stringify({foo: 'example', bar: 1}),
                ).toString('base64'),
            },
        },
        (err, res) => {
...
test('preParsing hook should run before parsing and be able to modify the payload', (t) => {
    ...
    fastify.addHook('preParsing', function (req, reply, payload, done) {
        ...
        modified.receivedEncodedLength = parseInt(
...
test('preParsing hooks should support encapsulation', (t) => {
    ...
    fastify.register((instance, opts, done) => {
        instance.addHook('preParsing', function (req, reply, payload, done) {
            const modified = new stream.Readable();
            modified.receivedEncodedLength =
                payload.receivedEncodedLength ||
...
test('preSerialization hooks should support encapsulation', (t) => {
    ...
    fastify.register((instance, opts, done) => {
        instance.addHook(
            'preSerialization',
            function (req, reply, payload, done) {
                payload.hello += '2';
