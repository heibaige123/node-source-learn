...
test('Ajv plugins array parameter', (t) => {
    ...
    fastify.inject(
        ...
        (err, res) => {
            ...
            t.equal(
                res.json().message,
                'body/foo should be <= 10@@@@should be multipleOf 2',
            );
        },
    );
});
...
test("serializer read validator's schemas", (t) => {
    ...
    const fastify = Fastify({
        schemaController: {
            bucket: function factory(storeInit) {
                t.notOk(
                    storeInit,
                    'is always empty because fastify.addSchema is not called',
                );
                return {
                    getSchemas() {
                        return {
                            [baseSchema.$id]: ajvInstance.getSchema(
                                baseSchema.$id,
                            ).schema,
                            [refSchema.$id]: ajvInstance.getSchema(
                                refSchema.$id,
                            ).schema,
                        };
                    },
    ...
    fastify.get('/', {
        schema: {
            response: {
                '2xx': ajvInstance.getSchema('http://example.com/schemas/ref')
                    .schema,
            },
        },
        handler(req, res) {
