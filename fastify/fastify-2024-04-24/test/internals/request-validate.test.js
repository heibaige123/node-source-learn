...
test('#compileValidationSchema', (subtest) => {
    ...
    subtest.test(
        'Validate function errors property should be null after validation when input is valid',
        async (t) => {
            const fastify = Fastify();
    ...
    subtest.test(
        'Validate function errors property should be an array of errors after validation when input is valid',
        async (t) => {
            const fastify = Fastify();
...
test('#getValidationFunction', (subtest) => {
                ...
                (req, reply) => {
                    ...
                    switch (params.id) {
                        ...
                        case 3:
                            ...
                            t.ok(
                                req.getValidationFunction('params')({id: 123}),
                            );
                            t.notOk(
                                req.getValidationFunction('params'({id: 1.2})),
                            );
                            break;
                        case 4:
    ...
    subtest.test(
        'Should not set a WeakMap if there is no schema',
        async (t) => {
            const fastify = Fastify();
                ...
                (req, reply) => {
                    ...
                    switch (params.id) {
                        ...
                        case 2:
                            ...
                            t.ok(
                                req.validateInput({foo: 'bar'}, 'querystring'),
                            );
                            break;
                        case 3:
                            t.notOk(
    ...
    subtest.test(
        ...
        async (t) => {
            ...
            fastify.get('/', (req, reply) => {
                t.equal(req[kRouteContext][kRequestCacheValidateFns], null);
                t.equal(
                    req.validateInput({hello: 'world'}, defaultSchema),
                    true,
                );
                t.type(req[kRouteContext][kRequestCacheValidateFns], WeakMap);
...
test('Nested Context', (subtest) => {
    ...
    subtest.test('Level_1', (tst) => {
        ...
        tst.test('#compileValidationSchema', (ntst) => {
            ...
            ntst.test(
                'Should compile the custom validation - nested with schema.headers',
                async (t) => {
                    const fastify = Fastify();
                    let called = false;
        ...
        tst.test('#getValidationFunction', (ntst) => {
                            ...
                            (req, reply) => {
                                ...
                                switch (params.id) {
                                    ...
                                    case 2:
                                        ...
                                        t.notOk(
                                            headerValidation({'x-foo': []}),
                                        );
                                        break;
                                    case 3:
                                        t.ok(
                                            req.getValidationFunction('params'),
                                        );
                                        t.ok(
                                            req.getValidationFunction('params')(
                                                {id: 123},
            ...
            ntst.test(
                'Should return a validation function - nested',
                async (t) => {
                    const fastify = Fastify();
                    let called = false;
                    const custom = ({schema, httpPart, url, method}) => {
            ...
            ntst.test(
                'Should per-route defined validation compiler',
                async (t) => {
                    const fastify = Fastify();
                    let validateParent;
                    let validateChild;
            ...
            ntst.test(
                ...
                async (t) => {
                    ...
                    const child = ({schema, httpPart, url, method}) => {
                        httpPart =
                            httpPart === 'querystring' ? 'query' : httpPart;
