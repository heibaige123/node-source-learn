...
const doGet = function (url) {
    return new Promise((resolve, reject) => {
        sget(
            {method: 'GET', url, followRedirects: false, agent},
            (err, response, body) => {
                if (err) {
                    reject(err);
                } else {
...
test('Once called, Reply should return an object with methods', (t) => {
    ...
    const request = {
        ...
        [kPublicRouteContext]: {config: context.config, schema: context.schema},
    };
...
test('reply.serialize should serialize payload with a custom serializer', (t) => {
    ...
    const reply = new Reply(response, {[kRouteContext]: context});
    reply.serializer(
        (x) => (customSerializerCalled = true) && JSON.stringify(x),
    );
    t.equal(reply.serialize({foo: 'bar'}), '{"foo":"bar"}');
...
test('reply.serialize should serialize payload with a context default serializer', (t) => {
    ...
    const response = {statusCode: 200};
    const context = {
        [kReplySerializerDefault]: (x) =>
            (customSerializerCalled = true) && JSON.stringify(x),
    };
    const reply = new Reply(response, {[kRouteContext]: context});
    t.equal(reply.serialize({foo: 'bar'}), '{"foo":"bar"}');
    t.equal(customSerializerCalled, true, 'custom serializer not called');
...
test('reply.serialize should serialize payload with Fastify instance', (t) => {
    ...
    fastify.route({
        ...
        handler: (req, reply) => {
            reply.send(reply.serialize({foo: 'bar'}));
        },
    });
            ...
            sget(
                {
                    ...
                    url:
                        'http://127.0.0.1:' +
                        fastify.server.address().port +
                        '/redirect-code-before-call-overwrite',
                },
                (err, response, body) => {
                    t.error(err);
                    t.equal(response.statusCode, 200);
...
test('plain string with custom json content type should NOT be serialized as json', (t) => {
    ...
    Object.keys(customSamples).forEach((path) => {
        fastify.get(`/${path}`, function (req, reply) {
            reply
                .type(customSamples[path].mimeType)
                .send(customSamples[path].sample);
        });
    });
        ...
        Object.keys(customSamples).forEach((path) => {
            sget(
                ...
                (err, response, body) => {
                    ...
                    t.equal(
                        response.headers['content-type'],
                        customSamples[path].mimeType + '; charset=utf-8',
                    );
...
test('non-string with custom json content type SHOULD be serialized as json', (t) => {
    ...
    const customSamples = {
        collectionjson: {
            mimeType: 'application/vnd.collection+json',
            sample: JSON.parse(
                '{"collection":{"version":"1.0","href":"http://api.example.com/people/"}}',
            ),
        },
        hal: {
            mimeType: 'application/hal+json',
            sample: JSON.parse(
                '{"_links":{"self":{"href":"https://api.example.com/people/1"}},"name":"John Doe"}',
            ),
        },
        jsonapi: {
            mimeType: 'application/vnd.api+json',
        ...
        jsonld: {
            mimeType: 'application/ld+json',
            sample: JSON.parse(
                '{"@context":"https://json-ld.org/contexts/person.jsonld","name":"John Doe"}',
            ),
        },
        siren: {
            mimeType: 'application/vnd.siren+json',
            sample: JSON.parse(
                '{"class":"person","properties":{"name":"John Doe"}}',
            ),
        },
    };
        ...
        Object.keys(customSamples).forEach((path) => {
            sget(
                ...
                (err, response, body) => {
                    ...
                    t.same(
                        body.toString(),
                        JSON.stringify(customSamples[path].sample),
                    );
                },
            );
        });
...
test('Reply should handle JSON content type with a charset', (t) => {
    ...
    fastify.get('/type-utf8', function (req, reply) {
        reply.type('application/json; charset=utf-8').send({hello: 'world'});
    });

    fastify.get('/type-utf16', function (req, reply) {
        reply.type('application/json; charset=utf-16').send({hello: 'world'});
    });

    fastify.get('/type-utf32', function (req, reply) {
        reply.type('application/json; charset=utf-32').send({hello: 'world'});
    });
    ...
    fastify.listen({port: 0}, (err) => {
        ...
        sget(
            ...
            (err, response, body) => {
                t.error(err);
                t.ok(response.headers['set-cookie']);
                t.strictSame(response.headers['set-cookie'], [
                    'one',
                    'two',
                    'three',
                    'four',
...
test('redirect to an invalid URL should not crash the server', async (t) => {
...

    {
        const {response, body} = await doGet(
            `http://127.0.0.1:${fastify.server.address().port}/redirect?useCase=1`,
        );
    ...
    }
    {
        const {response} = await doGet(
            `http://127.0.0.1:${fastify.server.address().port}/redirect?useCase=2`,
        );
        t.equal(response.statusCode, 302);
    ...
    {
        const {response} = await doGet(
            `http://127.0.0.1:${fastify.server.address().port}/redirect?useCase=3`,
        );
        t.equal(response.statusCode, 302);
...
test('invalid response headers should not crash the server', async (t) => {
    ...
    fastify.route({
        ...
        handler: (req, reply) => {
            reply.log.warn = function mockWarn(obj, message) {
                t.equal(
                    message,
                    'Invalid character in header content ["smile-encoded"]',
                    'only the first invalid header is logged',
                );
            };
    ...
    const {response, body} = await doGet(
        `http://127.0.0.1:${fastify.server.address().port}/bad-headers`,
    );
    t.equal(response.statusCode, 500);
    t.same(JSON.parse(body), {
        statusCode: 500,
    ...
    fastify.route({
        ...
        handler: (req, reply) => {
            reply.log.warn = function mockWarn(obj, message) {
                t.equal(
                    message,
                    'Invalid character in header content ["smile"]',
                    'only the first invalid header is logged',
                );
            };
...
test('invalid response headers and custom error handler', async (t) => {
    ...
    fastify.setErrorHandler(function (error, request, reply) {
        t.equal(
            error.message,
            'user land error',
            'custom error handler receives the error',
        );
...
test('reply.send will intercept ERR_HTTP_HEADERS_SENT and log an error message', (t) => {
    ...
    const log = {
        warn: (msg) => {
            t.equal(
                msg,
                'Reply was already sent, did you forget to "return reply" in the "/hello" (GET) route?',
            );
        },
    };
    ...
    const reply = new Reply(
        response,
        {
            [kRouteContext]: {onSend: null},
            raw: {url: '/hello', method: 'GET'},
        },
        log,
    );
