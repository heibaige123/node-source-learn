...
test('Reply#compileSerializationSchema', (t) => {
    ...
    t.test(
        ...
        async (t) => {
            ...
            fastify.get('/', {serializerCompiler: custom}, (req, reply) => {
                const input = {hello: 'world'};
                const first = reply.compileSerializationSchema(
                    schemaObj,
                    '201',
                );
                const second = reply.compileSerializationSchema(
                    schemaObj,
                    '201',
                );
    ...
    t.test('Should build a WeakMap for cache when called', async (t) => {
        ...
        fastify.get('/', (req, reply) => {
            ...
            t.equal(
                reply.compileSerializationSchema(getDefaultSchema())(input),
...
test('Reply#getSerializationFunction', (t) => {
        ...
        async (t) => {
            ...
            fastify.get(
                ...
                (req, reply) => {
                    ...
                    if (Number(id) === 1) {
                        const serialize4xx =
                            reply.getSerializationFunction('4xx');
                        const serialize201 =
                            reply.getSerializationFunction(201);
                        const serializeJson3xx = reply.getSerializationFunction(
                            '3xx',
                            'application/json',
                        );
                        const serializeUndefined =
                            reply.getSerializationFunction(undefined);
                        ...
                        t.equal(
                            serialize4xx(okInput4xx),
                            JSON.stringify(okInput4xx),
                        );
                        t.equal(
                            serialize201(okInput201),
                            JSON.stringify(okInput201),
                        );
                        t.equal(
                            serializeJson3xx(okInput3xx),
                        ...
                        try {
                            ...
                            t.equal(
                                err.message,
                                'The value "phone" cannot be converted to a number.',
                            );
                        }
            ...
            fastify.get(
                ...
                (req, reply) => {
                    ...
                    if (Number(id) === 1) {
                        ...
                        const serialize =
                            reply.getSerializationFunction(schemaObj);
    ...
    t.test(
        'Should not instantiate a WeakMap if it is not needed',
        async (t) => {
            const fastify = Fastify();
...
test('Reply#serializeInput', (t) => {
    ...
    t.test(
        ...
        async (t) => {
            ...
            t.same(result.json(), {
                ...
                error: 'Internal Server Error',
                message:
                    'Missing serialization function. Key "3xx:application/vnd.v1+json"',
            });
        },
    );
    ...
    t.test('Should instantiate a WeakMap after first call', async (t) => {
        ...
        fastify.get('/', (req, reply) => {
            ...
            t.equal(reply[kRouteContext][kReplyCacheSerializeFns], null);
            t.equal(
                reply.serializeInput(input, getDefaultSchema()),
                JSON.stringify(input),
            );
            t.type(reply[kRouteContext][kReplyCacheSerializeFns], WeakMap);
