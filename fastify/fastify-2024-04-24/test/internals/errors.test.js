...
test('FST_ERR_REP_ALREADY_SENT', (t) => {
    ...
    t.equal(
        error.message,
        'Reply was already sent, did you forget to "return reply" in "/hello" (GET)?',
    );
    t.equal(error.statusCode, 500);
    t.ok(error instanceof Error);
});
...
test('FST_ERR_FORCE_CLOSE_CONNECTIONS_IDLE_NOT_AVAILABLE', (t) => {
    t.plan(5);
    const error =
        new errors.FST_ERR_FORCE_CLOSE_CONNECTIONS_IDLE_NOT_AVAILABLE();
    t.equal(error.name, 'FastifyError');
...
test('FST_ERR_ROUTE_BODY_VALIDATION_SCHEMA_NOT_SUPPORTED', (t) => {
    t.plan(5);
    const error =
        new errors.FST_ERR_ROUTE_BODY_VALIDATION_SCHEMA_NOT_SUPPORTED();
    t.equal(error.name, 'FastifyError');
...
test('Ensure that all errors are in Errors.md TOC', (t) => {
    ...
    const errorsMd = readFileSync(
        resolve(__dirname, '../../docs/Reference/Errors.md'),
        'utf8',
    );
...
test('Ensure that all errors are in Errors.md documented', (t) => {
    ...
    for (const key of exportedKeys) {
        if (errors[key].name === 'FastifyError') {
            t.ok(
                errorsMd.includes(
                    `<a id="${key.toLowerCase()}">${key.toUpperCase()}</a>`,
                ),
                key,
            );
        }
