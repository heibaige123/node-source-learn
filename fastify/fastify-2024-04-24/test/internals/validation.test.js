...
['compileSchemasForValidation', 'compileSchemasForSerialization'].forEach(
    (func) => {
...
test('build schema - output schema', (t) => {
    ...
    validation.compileSchemasForSerialization(
        opts,
        ({schema, method, url, httpPart}) => ajv.compile(schema),
    );
...
test('build schema - uppercased headers are not included', (t) => {
    ...
    validation.compileSchemasForValidation(
        ...
        ({schema, method, url, httpPart}) => {
            t.notOk(
                'Content-Type' in schema.properties,
