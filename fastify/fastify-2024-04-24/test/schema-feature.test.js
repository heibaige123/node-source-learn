...
test('Cross schema reference with encapsulation references', (t) => {
    ...
    fastify.register(
        (instance, opts, done) => {
            ...
            instance.get(
                '/get',
                {schema: {response: {200: deepClone(multipleRef)}}},
                () => {},
            );
            instance.get(
                '/double-get',
            ...
            instance.post(
                '/post',
                {schema: {body: multipleRef, response: {200: multipleRef}}},
                () => {},
            );
            instance.post(
                '/double',
                {schema: {response: {200: {$ref: 'encapsulation'}}}},
                () => {},
            );
            done();
        },
    ...
    fastify.post(
        '/post',
        {schema: {body: refItem, response: {200: refItem}}},
        () => {},
    );
    fastify.get(
        '/get',
        {schema: {params: refItem, response: {200: refItem}}},
        () => {},
    );
...
test('Check how many AJV instances are built #1', (t) => {
    ...
    fastify.ready((err) => {
        ...
        instances.forEach((i) => {
            ...
            t.equal(
                i.validatorCompiler.checkPointer,
                true,
                'validator is only one for all the instances',
            );
        });
    });
});
...
test('Check how many AJV instances are built #2 - verify validatorPool', (t) => {
    ...
    fastify.register(function sibling3(instance, opts, done) {
        ...
        instance.ready(() => {
            ...
            t.notOk(
                instance.validatorCompiler.sharedPool,
                'this context has its own compiler',
            );
        });
        done();
    });
...
test('Schema controller setter', (t) => {
    ...
    try {
        ...
        t.equal(
            err.message,
            "schemaController.bucket option should be a function, instead got 'object'",
        );
    }
});
...
test('Schema controller bucket', (t) => {
    ...
    fastify.ready((err) => {
        ...
        t.equal(
            builtBucket,
            4,
            'one bucket built for every register call + 1 for the root instance',
        );
    });
});
            ...
            compilersFactory: {
                buildValidator: function (externalSchemas) {
                    ...
                    t.equal(
                        schemaKeys.length,
                        2,
                        'Contains same number of schemas',
                    );
                    t.hasStrict(
...
test('Should coerce the array if the default validator is used', async (t) => {
    ...
    try {
        ...
        t.same(
            res.json(),
            {msg: ['string']},
            'Should coerce the string into array',
        );
    } catch (err) {
        t.error(err);
    }
