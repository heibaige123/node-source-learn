...
function testHandlerOrBeforeHandlerHook(test, hookOrHandler) {
    const idx =
        hookOrHandler === 'handler'
            ? lifecycleHooks.indexOf('preHandler')
            : lifecycleHooks.indexOf(hookOrHandler);
    ...
    test(`Hijacking inside ${hookOrHandler} skips all the following hooks and handler execution`, (t) => {
        ...
        test('Sending a response using reply.raw => onResponse hook is called', (t) => {
            ...
            previousHooks.forEach((h) =>
                app.addHook(h, async (req, reply) =>
                    t.pass(`${h} should be called`),
                ),
            );
                ...
                const client = net.createConnection(
                    ...
                    () => {
                        ...
                        client.on('end', function () {
                            t.match(
                                chunks,
                                new RegExp(`hello from ${hookOrHandler}`, 'i'),
                            );
                            t.end();
                        });
                    },
