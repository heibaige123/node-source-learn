...
t.test('logging', (t) => {
    ...
    t.test(
        ...
        async (t) => {
            ...
            fastify.get('/400', function (req, reply) {
                t.ok(req.log);
                reply.send(
                    Object.assign(new Error('a 400 error'), {statusCode: 400}),
                );
            });
            fastify.get('/503', function (req, reply) {
                t.ok(req.log);
                reply.send(
                    Object.assign(new Error('a 503 error'), {statusCode: 503}),
                );
            });
    ...
    t.test(
        'should not log the error if error handler is defined and it does not error',
        async (t) => {
            const lines = [
                {msg: /Server listening at/},
                {level: 30, msg: 'incoming request'},
    ...
    t.test(
        'should not log incoming request and outgoing response for 404 onBadUrl when disabled',
        async (t) => {
            t.plan(3);
            const stream = split(JSON.parse);
            const fastify = Fastify({
