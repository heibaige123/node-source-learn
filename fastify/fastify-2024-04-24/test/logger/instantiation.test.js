...
t.test('logger instantiation', (t) => {
    ...
    t.test('Wrap IPv6 address in listening log message', async (t) => {
        ...
        if (ipv6 === undefined) {
...

            {
                ...
                t.same(
                    line.msg,
                    `Server listening at http://[${ipv6}]:${fastify.server.address().port}`,
                );
            }
        }
    });
    ...
    t.test('Do not wrap IPv4 address', async (t) => {
...

        {
            ...
            t.same(
                line.msg,
                `Server listening at http://127.0.0.1:${fastify.server.address().port}`,
            );
        }
    });
    ...
    t.test('file option', async (t) => {
        ...
        await fastify.listen({port: 0, host: localhost});
        await request(
            `http://${localhostForURL}:` + fastify.server.address().port,
        );
    ...
    t.test(
        'should throw in case a partially matching logger is provided',
        async (t) => {
            t.plan(1);
    ...
    t.test(
        'should throw in case the external logger provided does not have a child method',
        async (t) => {
            t.plan(1);
