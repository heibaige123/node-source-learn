...
test('request.routeOptions should be immutable', (t) => {
    ...
    const handler = function (req, res) {
        ...
        for (const key of Object.keys(req.routeOptions)) {
            if (
                typeof req.routeOptions[key] === 'object' &&
                req.routeOptions[key] !== null
            ) {
                t.fail(
                    'Object.freeze must run recursively on nested structures to ensure that routeOptions is immutable.',
                );
            }
        }
