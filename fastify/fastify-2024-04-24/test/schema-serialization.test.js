...
test('Different content types', (t) => {
    ...
    fastify.inject(
        {method: 'GET', url: '/', headers: {Accept: 'application/vnd.v1+json'}},
        (err, res) => {
            t.error(err);
            t.equal(
                res.payload,
    ...
    fastify.inject(
        {method: 'GET', url: '/', headers: {Accept: 'application/vnd.v2+json'}},
        (err, res) => {
            t.error(err);
            t.equal(
                res.payload,
                JSON.stringify({fullName: 'Jhon Smith', phone: '01090000000'}),
            );
    ...
    fastify.inject(
        {method: 'GET', url: '/', headers: {Accept: 'application/vnd.v3+json'}},
        (err, res) => {
            t.error(err);
            t.equal(
                res.payload,
    ...
    fastify.inject(
        {method: 'GET', url: '/', headers: {Accept: 'application/vnd.v4+json'}},
        (err, res) => {
            t.error(err);
    ...
    fastify.inject(
        {method: 'GET', url: '/', headers: {Accept: 'application/vnd.v5+json'}},
        (err, res) => {
            t.error(err);
            t.equal(
                res.payload,
                JSON.stringify({content: 'Processing exclusive content'}),
            );
    ...
    fastify.inject(
        {method: 'GET', url: '/', headers: {Accept: 'application/vnd.v6+json'}},
        (err, res) => {
            t.error(err);
            t.equal(
                res.payload,
    ...
    fastify.inject(
        {method: 'GET', url: '/', headers: {Accept: 'application/vnd.v7+json'}},
        (err, res) => {
            t.error(err);
...
test('Shared schema should be pass to serializer and validator ($ref to shared schema /definitions)', (t) => {
    ...
    fastify.post(
        ...
        (req, reply) => {
            reply.send(
                locations.map((_) =>
                    Object.assign({serializer: 'remove me'}, _),
                ),
            );
        },
    );
            ...
            fastify.inject(
                ...
                (err, res) => {
                    ...
                    t.same(res.json(), {
                        error: 'Bad Request',
                        message:
                            'body/0/location/email must match format "email"',
                        statusCode: 400,
                        code: 'FST_ERR_VALIDATION',
                    });
                },
...
test('Custom setSerializerCompiler', (t) => {
    ...
    fastify.register(
        (instance, opts, done) => {
            ...
            t.ok(
                instance.serializerCompiler,
                'the serializer is set by the parent',
            );
            done();
        },
        {prefix: '/foo'},
    ...
    fastify.get(
        '/',
        {
            ...
            serializerCompiler: ({schema, method, url, httpPart}) => {
                ...
                return () => {
                    t.fail(
                        'the serializer must not be called when there is a reply serializer',
                    );
                    return 'fail';
                };
            },
...
test('error in custom schema serialize compiler, throw FST_ERR_SCH_SERIALIZATION_BUILD error', (t) => {
    ...
    fastify.ready((err) => {
        t.equal(
            err.message,
            'Failed building the serialization schema for GET: /, due to error CUSTOM_ERROR',
        );
        t.equal(err.statusCode, 500);
