...
['trace', 'debug', 'info', 'warn', 'error', 'fatal'].forEach((logLevel) => {
    ...
    expectType<void>(
        fastify<
            ...
            ServerResponse,
            FastifyLoggerInstance
        >().log[logLevel as LogLevel]({foo: 'bar'}),
    );
    expectType<void>(
        fastify<
            Server,
            ...
            ServerResponse,
            FastifyLoggerInstance
        >().log[logLevel as LogLevel](new Error()),
    );
    expectType<void>(
        fastify<
            Server,
            ...
            ServerResponse,
            FastifyLoggerInstance
        >().log[logLevel as LogLevel](new Foo()),
    );
});
...
const serverAutoInferredSerializerResponseObjectOption = fastify({
    logger: {
        serializers: {
            res(ServerResponse) {
                expectType<ResSerializerReply<Server, FastifyReply>>(
                    ServerResponse,
                );
                expectAssignable<
...
expectType<FastifyBaseLogger>(
    serverAutoInferredSerializerResponseObjectOption.log,
);
...
expectType<FastifyLoggerInstance>(
    childParent.child({}, {redact: ['pass', 'pin']}),
);
expectType<FastifyLoggerInstance>(
    childParent.child({}, {serializers: {key: () => {}}}),
);
expectType<FastifyLoggerInstance>(
    childParent.child(
        {},
        {level: 'info', redact: ['pass', 'pin'], serializers: {key: () => {}}},
    ),
);
