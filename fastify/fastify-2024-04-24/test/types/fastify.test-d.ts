...
expectAssignable<FastifyInstance>(
    fastify({serializerOpts: {rounding: 'ceil'}}),
);
expectAssignable<FastifyInstance>(
...
expectAssignable<FastifyInstance>(
    fastify({querystringParser: () => ({foo: {bar: 'fuzz'}})}),
);
expectAssignable<FastifyInstance>(
    fastify({querystringParser: () => ({foo: ['bar', 'fuzz']})}),
);
...
// Thenable
expectAssignable<PromiseLike<FastifyInstance>>(
    fastify({return503OnClosing: true}),
);
fastify().then((fastifyInstance) =>
