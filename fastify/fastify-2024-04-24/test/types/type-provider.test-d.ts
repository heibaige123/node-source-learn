...
interface JsonSchemaToTsProvider extends FastifyTypeProvider {
    output: this['input'] extends JSONSchema
        ? FromSchema<this['input']>
        : unknown;
}
...
expectAssignable(
    server.withTypeProvider<TypeBoxProvider>().get(
        ...
        async (_, res) => {
            ...
            expectType<(payload?: {error: string}) => typeof res>(
                res.code(500).send,
            );
...
expectAssignable(
    server.withTypeProvider<JsonSchemaToTsProvider>().get(
        ...
        (_, res) => {
            ...
            expectType<(payload?: number) => typeof res>(res.code(400).send);
            expectType<
                (payload?: {[x: string]: unknown; error?: string}) => typeof res
            >(res.code(500).send);
            expectError<(payload?: unknown) => typeof res>(res.code(200).send);
        },
    ),
...
expectAssignable(
    server
        .withTypeProvider<AuxiliaryPluginProvider>()
        .register(plugin)
        .get(
            '/',
            {
