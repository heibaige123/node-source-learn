...
type ExpectedGetValidationFunction = (input: {
    [key: string]: unknown;
}) => boolean;
...
const getHandler: RouteHandler = function (request, _reply) {
    ...
    expectType<FastifyRequestContext<ContextConfigDefault>['config']>(
    ...
    expectType<
        ContextConfigDefault & FastifyRouteConfig & FastifyContextConfig
    >(request.routeOptions.config);
    ...
    expectType<RequestHeadersDefault & RawRequestDefaultExpression['headers']>(
        request.headers,
    );
    request.headers = {};
    ...
    expectType<
        (Error & {validation: any; validationContext: string}) | undefined
