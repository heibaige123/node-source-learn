...
const testPluginOpts: FastifyPluginCallback<TestOptions> = function (
    instance,
    opts,
    done,
) {};
const testPluginOptsAsync: FastifyPluginAsync<TestOptions> = async function (
    instance,
    opts,
) {};
...
expectAssignable<FastifyInstance>(
    fastify().register(testPluginOpts, {option1: '', option2: true}),
);
expectAssignable<FastifyInstance>(
    fastify().register(testPluginOptsAsync, {option1: '', option2: true}),
);
...
expectAssignable<FastifyInstance>(
    fastify().register(import('./dummy-plugin'), {foo: 1}),
);
...
expectAssignable<FastifyInstance>(
    fastify().register(function (instance, opts): Promise<void> {
...
expectError(
    fastify().register(function (instance, opts, done) {}, {logLevel: ''}),
); // must use a valid logLevel
...
expectAssignable<PromiseLike<undefined>>(
    httpsServer.register(testPluginOptsWithType),
);
expectAssignable<PromiseLike<undefined>>(
    httpsServer.register(testPluginOptsWithTypeAsync),
);
expectAssignable<PromiseLike<undefined>>(
    httpsServer.register(testPluginOptsWithType, {prefix: '/test'}),
);
expectAssignable<PromiseLike<undefined>>(
    httpsServer.register(testPluginOptsWithTypeAsync, {prefix: '/test'}),
);

async function testAsync(): Promise<void> {
    await httpsServer.register(testPluginOpts).register(testPluginOpts);
}
