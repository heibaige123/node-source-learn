...
const routeHandlerWithReturnValue: RouteHandlerMethod = function (
    request,
    reply,
) {
    expectType<FastifyInstance>(this);
...
['GET', 'POST', 'PUT', 'PATCH', 'HEAD', 'DELETE', 'OPTIONS'].forEach(
    (method) => {
        // route method
        ...
        const lowerCaseMethod: LowerCaseHTTPMethods =
            method.toLowerCase() as LowerCaseHTTPMethods;

        // method as method
        expectType<FastifyInstance>(
            fastify()[lowerCaseMethod]('/', routeHandler),
        );
        expectType<FastifyInstance>(
            fastify()[lowerCaseMethod]('/', {}, routeHandler),
        );
        expectType<FastifyInstance>(
            fastify()[lowerCaseMethod]('/', {handler: routeHandler}),
        );
        ...
        fastify()[lowerCaseMethod]<
            RouteGeneric,
            RouteSpecificContextConfigType
        >('/', {config: {foo: 'bar', bar: 100, extra: true}}, (req, res) => {
            expectType<BodyInterface>(req.body);
            expectType<QuerystringInterface>(req.query);
            expectType<ParamsInterface>(req.params);
            expectType<http.IncomingHttpHeaders & HeadersInterface>(
                req.headers,
            );
        ...
        fastify().route<RouteGeneric>({
            ...
            onRequest: (req, res, done) => {
                // these handlers are tested in `hooks.test-d.ts`
                expectType<BodyInterface>(req.body);
                expectType<QuerystringInterface>(req.query);
                expectType<ParamsInterface>(req.params);
                expectType<http.IncomingHttpHeaders & HeadersInterface>(
                ...
                expectType<number>(req.context.config.bar);
                expectType<string>(req.context.config.url);
                expectType<HTTPMethods | HTTPMethods[]>(
            ...
            preParsing: (req, res, payload, done) => {
                ...
                expectAssignable<
                    (err?: FastifyError | null, res?: RequestPayload) => void
                >(done);
