...
type DefaultSerializationFunction = (payload: {
    [key: string]: unknown;
}) => string;
...
const getHandler: RouteHandlerMethod = function (_request, reply) {
    ...
    expectType<FastifyReplyContext<ContextConfigDefault>['config']>(
    ...
    expectType<
        <Code extends number>(
            statusCode: Code,
        ) => DefaultFastifyReplyWithCode<Code>
    >(reply.code);
    expectType<
        <Code extends number>(
            statusCode: Code,
    ...
    >(reply.status);
    expectType<(payload?: unknown) => FastifyReply>(
        reply.code(100 as number).send,
    );
    ...
    expectAssignable<(values: {[key: string]: any}) => FastifyReply>(
        reply.headers,
    );
    expectAssignable<(key: string) => number | string | string[] | undefined>(
        reply.getHeader,
    );
    expectAssignable<
    ...
    expectType<(fn: (payload: any) => string) => FastifyReply>(
        reply.serializer,
    );
    expectType<(payload: any) => string | ArrayBuffer | Buffer>(
        reply.serialize,
    );
    expectType<(fulfilled: () => void, rejected: (err: Error) => void) => void>(
        reply.then,
    );
    expectType<
        (
    ...
    expectAssignable<(httpStatus: string) => DefaultSerializationFunction>(
        reply.getSerializationFunction,
    );
    expectAssignable<
        (schema: {[key: string]: unknown}) => DefaultSerializationFunction
    >(reply.getSerializationFunction);
    expectAssignable<
        (
...
expectError(
    server.get<ReplyPayload>(
        '/get-generic-return-error',
        async function handler(request, reply) {
            return {foo: 'bar'};
        },
    ),
);
server.get<ReplyUnion>(
    '/get-generic-union-send',
    async function handler(request, reply) {
        if ((0 as number) === 0) {
...
expectError(
    server.get<ReplyUnion>(
        '/get-generic-union-send-error-1',
        async function handler(request, reply) {
...
expectError(
    server.get<ReplyUnion>(
        '/get-generic-union-send-error-2',
        async function handler(request, reply) {
...
expectError(
    server.get<ReplyUnion>(
        '/get-generic-union-return-error-1',
        async function handler(request, reply) {
...
expectError(
    server.get<ReplyUnion>(
        '/get-generic-union-return-error-2',
        async function handler(request, reply) {
            return {error: 500};
        },
    ),
);
server.get<ReplyHttpCodes>(
    '/get-generic-http-codes-send',
    async function handler(request, reply) {
...
expectError(
    server.get<ReplyHttpCodes>(
        '/get-generic-http-codes-send-error-1',
        async function handler(request, reply) {
...
expectError(
    server.get<ReplyHttpCodes>(
        '/get-generic-http-codes-send-error-2',
        async function handler(request, reply) {
...
expectError(
    server.get<ReplyHttpCodes>(
        '/get-generic-http-codes-send-error-3',
        async function handler(request, reply) {
...
expectError(
    server.get<ReplyHttpCodes>(
        '/get-generic-http-codes-send-error-4',
        async function handler(request, reply) {
...
expectError(
    server.get<ReplyHttpCodes>(
        '/get-generic-http-codes-send-error-5',
        async function handler(request, reply) {
            reply.code(401).send({foo: 123});
        },
    ),
);
server.get<ReplyArrayPayload>(
    '/get-generic-array-send',
    async function handler(request, reply) {
        reply.code(200).send(['']);
    },
);
expectError(
    server.get<InvalidReplyHttpCodes>(
        'get-invalid-http-codes-reply-error',
        async function handler(request, reply) {
            reply.code(200).send('');
        },
    ),
