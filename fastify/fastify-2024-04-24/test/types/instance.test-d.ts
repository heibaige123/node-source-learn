...
async function asyncFastifyErrorHandler(
...
server.setErrorHandler<CustomError, ReplyPayload>(
...
async function notFoundAsyncHandler(
...
server.setNotFoundHandler(
    {preHandler: notFoundpreHandlerAsyncHandler},
    notFoundHandler,
);
server.setNotFoundHandler(
...
server.setNotFoundHandler(
    {preValidation: notFoundpreValidationAsyncHandler},
    notFoundHandler,
);
server.setNotFoundHandler(
    {
...
function invalidSchemaController(
...
expectAssignable<void>(
    server.listen(3000, '', 0, (err, address) => {
...
expectAssignable<PromiseLike<string>>(
    server.listen({
        port: 3000,
...
expectAssignable<PromiseLike<string>>(
    server.listen({port: 3000, host: '0.0.0.0', backlog: 42}),
);
expectAssignable<PromiseLike<string>>(
    server.listen({port: 3000, host: '0.0.0.0', backlog: 42, exclusive: true}),
);
expectAssignable<PromiseLike<string>>(
    server.listen({port: 3000, host: '::/0', ipv6Only: true}),
);
...
expectAssignable<void>(
    server.listen(
        {port: 3000, host: '0.0.0.0', backlog: 42, exclusive: true},
        () => {},
    ),
);
expectAssignable<void>(
...
expectAssignable<void>(
    server.routing(
        {} as RawRequestDefaultExpression,
        {} as RawReplyDefaultExpression,
    ),
);

expectType<FastifyInstance>(
    fastify().get<RouteGenericInterface, {contextKey: string}>('/', {
        handler: () => {},
...
expectType<FastifyInstance>(
    fastify().get('/', {
        ...
        childLoggerFactory: (logger, bindings, opts, req) => {
            expectAssignable<FastifyBaseLogger>(
                server.childLoggerFactory(logger, bindings, opts, req),
            );
...
expectType<FastifyBodyParser<string>>(
    server.getDefaultJsonParser('ignore', 'error'),
);
...
server.decorateRequest<(x: string, y: number) => void>(
    'test',
...
expectError(
    server.decorate<(myNumber: number) => number>(
        'test',
        function (myNumber: number): string {
            return '';
        },
    ),
);
...
expectError(
    server.decorate('typedTestMethod', async function (x) {
        return 'foo';
    }),
);
...
expectError(
    server.decorateRequest('typedTestRequestMethod', async function (x) {
        return 'foo';
    }),
);
...
expectError(
    server.decorateReply('typedTestReplyMethod', async function (x) {
        return 'foo';
    }),
);
...
expectType<boolean>(
    server.hasConstraintStrategy(versionConstraintStrategy.name),
);

expectType<boolean>(server.hasPlugin(''));

expectAssignable<
    DefaultRoute<RawRequestDefaultExpression, RawReplyDefaultExpression>
>(server.getDefaultRoute());

expectType<FastifySchemaCompiler<any> | undefined>(server.validatorCompiler);
expectType<FastifySerializerCompiler<any> | undefined>(
    server.serializerCompiler,
);
