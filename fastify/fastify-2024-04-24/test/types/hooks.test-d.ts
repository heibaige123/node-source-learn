...
server.addHook('preParsing', function (request, reply, payload, done) {
    ...
    expectAssignable<(err?: FastifyError | null, res?: RequestPayload) => void>(
        done,
    );
    expectAssignable<(err?: NodeJS.ErrnoException) => void>(done);
    expectType<void>(done(new Error()));
...
server.addHook<TestPayloadType>(
    'preSerialization',
...
server.addHook('onRoute', function (opts) {
    expectType<FastifyInstance>(this);
    expectType<
        RouteOptions & {routePath: string; path: string; prefix: string}
    >(opts);
});
...
server.addHook<TestPayloadType>(
    'preSerialization',
    async function (request, reply, payload) {
        expectType<FastifyInstance>(this);
        expectType<FastifyRequest>(request);
        expectType<FastifyReply>(reply);
        expectType<TestPayloadType>(payload); // we expect this to be unknown when not specified like in the previous test
...
type CustomContextRequest = FastifyRequest<
    any,
    any,
    any,
    any,
    ...
    any
>;
type CustomContextReply = FastifyReply<
    any,
    any,
    any,
    any,
...
expectError(
    server.addHook(
        'preSerialization',
        async function (request, reply, payload, done) {},
    ),
);
expectError(
