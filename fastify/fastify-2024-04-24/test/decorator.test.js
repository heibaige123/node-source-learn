...
test('decorate should throw if a declared dependency is not present', (t) => {
    ...
    fastify.register((instance, opts, done) => {
        try {
            ...
            t.same(
                e.message,
                "The decorator is missing dependency 'dependency'.",
            );
        }
        done();
    });
...
test('decorate should throw if declared dependency is not array', (t) => {
    ...
    fastify.register((instance, opts, done) => {
        try {
            ...
            t.same(
                e.message,
                "The dependencies of decorator 'test' must be of type Array.",
            );
        }
        done();
    });
        ...
        sget(
            {
                method: 'GET',
                url:
                    'http://localhost:' + fastify.server.address().port + '/no',
            },
            (err, response, body) => {
                t.error(err);
                t.equal(response.statusCode, 200);
...
test('hasRequestDecorator', (t) => {
    ...
    t.test(
        'should check if the given request decoration already exist when null',
        (t) => {
            t.plan(2);
            const fastify = Fastify();
...
test('hasReplyDecorator', (t) => {
    ...
    t.test(
        'should check if the given reply decoration already exist when null',
        (t) => {
            t.plan(2);
            const fastify = Fastify();
...
test('decorateRequest with dependencies', (t) => {
    ...
    if (
        ...
        t.doesNotThrow(() =>
            app.decorateRequest('decorator2', decorator2, ['decorator1']),
        );
