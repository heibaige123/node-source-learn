        ...
        instance.register(
            fp((i, o, n) => {
                try {
                    ...
                    t.equal(
                        e.message,
                        "The decorator is missing dependency 'test'.",
                    );
                }
                n();
            }),
                ...
                fastify.register(
                    fp(
                        (fastify, opts, done) => {
                            t.equal(
                                fastify.pluginName,
                                'fastify -> plugin-A -> plugin-AB -> plugin-AC',
                            );
                            done();
                        },
                        {name: 'plugin-AC'},
    ...
    fastify.register(
        fp(
            (fastify, opts, done) => {
                t.equal(
                    fastify.pluginName,
                    'fastify -> plugin-A -> plugin-AB -> plugin-AC -> plugin-B',
                );
                done();
            },
            {name: 'plugin-B'},
...
test('approximate a plugin name when no meta data is available', (t) => {
    ...
    fastify.register((fastify, opts, done) => {
        ...
        fastify.register((fastify, opts, done) => {
            // B
            t.equal(
                fastify.pluginName.startsWith('(fastify, opts, done)'),
                true,
            );
...
test('approximate a plugin name also when fastify-plugin has no meta data', (t) => {
    ...
    fastify.register(
        fp((fastify, opts, done) => {
            ...
            fastify.register(
                fp(function B(fastify, opts, done) {
                    // function has name
