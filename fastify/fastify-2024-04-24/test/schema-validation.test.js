    ...
    fastify.register((instance, opts, done) => {
        ...
        instance.register((instance, opts, done) => {
            ...
            instance.post('/three', {
                ...
                handler(req, reply) {
                    t.same(
                        instance.validatorCompiler,
                        validator,
                        'the route validator does not change the instance one',
                    );
...
test('Triple $ref with a simple $id', (t) => {
    ...
    fastify.inject(
        ...
        (err, res) => {
            ...
            t.same(
                res.json().message,
                "body must have required property 'foo'",
            );
        },
    );
});
...
test('Custom AJV settings - pt2', (t) => {
    ...
    fastify.post('/', {
        ...
        handler: (req, reply) => {
            t.fail(
                'the handler is not called because the "12" is not coerced to number',
            );
        },
    });
...
test('Custom validator builder override by custom validator compiler', async (t) => {
    ...
    const ajv1 = new AJV(ajvDefaults).addKeyword({
        keyword: 'extended_one',
        type: 'object',
        validator: () => true,
    });
    const ajv2 = new AJV(ajvDefaults).addKeyword({
        keyword: 'extended_two',
        type: 'object',
        validator: () => true,
    });
    const fastify = Fastify({
        schemaController: {
            compilersFactory: {
                buildValidator: () => (routeSchemaDef) =>
                    ajv1.compile(routeSchemaDef.schema),
            },
        },
    });

    fastify.setValidatorCompiler((routeSchemaDef) =>
        ajv2.compile(routeSchemaDef.schema),
    );
...
test('Custom validator builder override by custom validator compiler in child instance', async (t) => {
    ...
    fastify.register((embedded, _opts, done) => {
        embedded.setValidatorCompiler((routeSchemaDef) =>
            ajv2.compile(routeSchemaDef.schema),
        );
