...
test('getParser', (t) => {
    test('should return matching parser', (t) => {
        ...
        t.equal(
            fastify[keys.kContentTypeParser].getParser('application/t+xml').fn,
            second,
        );
        t.equal(
            fastify[keys.kContentTypeParser].getParser('image/png').fn,
            first,
        );
        t.equal(
            fastify[keys.kContentTypeParser].getParser('text/html').fn,
            third,
        );
    });
    ...
    test('should prefer content type parser with string value', (t) => {
        ...
        t.equal(
            fastify[keys.kContentTypeParser].getParser('image/gif').fn,
            second,
        );
        t.equal(
            fastify[keys.kContentTypeParser].getParser('image/png').fn,
...
test('remove', (t) => {
    test('should remove default parser', (t) => {
        ...
        t.notOk(
            contentTypeParser.parserList.find(
                (parser) => parser === 'application/json',
            ),
        );
    });
    ...
    test('should remove RegExp parser', (t) => {
        ...
        t.notOk(
            contentTypeParser.parserRegExpList.find(
                (parser) => parser.toString() === /^text\/*/.toString(),
            ),
        );
    });
    ...
    test('should throw an error if content type is neither string nor RegExp', (t) => {
        ...
        t.throws(
            () => fastify[keys.kContentTypeParser].remove(12),
            FST_ERR_CTP_INVALID_TYPE,
        );
    });
...
test('content-type match parameters - string 2', async (t) => {
    ...
    fastify.addContentTypeParser(
        'text/plain; charset=utf8; foo=bar',
        function (request, body, done) {
            t.fail("shouldn't be called");
            done(null, body);
        },
