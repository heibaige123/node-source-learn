...
test('fastify instance should contain default errorHandler', (t) => {
    ...
    t.same(
        Object.getOwnPropertyDescriptor(fastify, 'errorHandler').set,
        undefined,
    );
});
...
test('fastify instance should contain default childLoggerFactory', (t) => {
    ...
    t.same(
        Object.getOwnPropertyDescriptor(fastify, 'childLoggerFactory').set,
        undefined,
    );
});
...
test('ready should reject in order when called multiply times (promises only)', async (t) => {
    ...
    const promises = [1, 2, 3, 4, 5].map((id) =>
        app.ready().catch(() => result.push(id)),
    );
