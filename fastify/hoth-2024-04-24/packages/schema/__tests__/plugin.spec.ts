...
describe('hoth schema plugin', () => {
    it('init schema', async () => {
        const originSchemaStr = await fs.readFile(
            path.resolve(__dirname, './sample/schema.json'),
            'utf-8',
        );
        ...
        expect(addSchemaSpy).toHaveBeenCalledTimes(
            Object.keys(originSchema).length,
        );
    });
});
