...
function createInfo(
    ...
    const conf =
        require(join(repoTemplatesDir, name, 'package.json'))['hoth-cli'] || {}; // eslint-disable-line
    ...
    return {
        ...
        logInstructions(dir: string) {
            ...
            console.log(
                `run '${chalk.bold('npm run build')}' to compile the application`,
            );
            console.log(
                `run '${chalk.bold('npm run dev')}' to start the application`,
            );
...
function generate(
    dir: string,
    templateInfo: ReturnType<typeof createInfo>,
...
export async function cli(args: string[]) {
    ...
    if (opts.repo) {
        const repo =
            opts.repo.startsWith('ssh:') ||
            opts.repo.startsWith('http:') ||
            opts.repo.startsWith('https:')
                ? opts.repo
        ...
        if (existsSync(repoTemplatesDir)) {
            try {
                ...
                console.warn(
                    'failed to git pull latest templates. Maybe use to use old templates',
                );
            }
        } else {
            /* istanbul ignore else */
            ...
            try {
                ...
                console.error(
                    'fail to clone template repo. please make sure installed git and connected internet ',
                );
                console.error(e);
            }
        }
