...
describe('@hoth/logger parser', function () {
    ...
    it('parse normal log warmup skip', function () {
        // eslint-disable-next-line max-len
        let message =
            'NOTICE: 2021-08-09 13:28:35 [-:-] errno[-] status[200] logId[warmup] pid[14871] uri[/app/other] cluster[-] idc[-] product[quickstart]';
        const log = parse(message);
    ...
    it('cluster as app', function () {
        // eslint-disable-next-line max-len
        const message =
            'ERROR: 2021-08-09 14:08:16 [-:-] errno[-] status[-] cluster[test]';
        const log = parse(message);
