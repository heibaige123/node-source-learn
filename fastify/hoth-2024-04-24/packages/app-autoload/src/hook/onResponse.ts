...
export default async function (req: FastifyRequest, reply: FastifyReply) {
    ...
    const serializationStartTime =
        req[serializationTimeSym] || req[sendStartTimeSym];

    req.log.addNotice(
        'initTime',
        (parseStartTime - req[reqStartTimeSym]).toFixed(1),
    );
    req.log.addNotice(
        'parseTime',
        (req[validateStartTimeSym] - parseStartTime).toFixed(1),
    );
    req.log.addNotice(
        'validateTime',
        (req[handlerStartTimeSym] - req[validateStartTimeSym]).toFixed(1),
    );
    req.log.addNotice(
        'handleTime',
        (serializationStartTime - req[handlerStartTimeSym]).toFixed(1),
    );
    req.log.addNotice(
        'serialTime',
        (req[sendStartTimeSym] - serializationStartTime).toFixed(1),
    );
